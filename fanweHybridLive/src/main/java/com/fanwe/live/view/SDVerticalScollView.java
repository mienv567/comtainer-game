...
public class SDVerticalScollView extends RelativeLayout {
    ...
    public TextView getTv_user_number_left() {
        return tv_user_number_left;
    }

    private TextView tv_user_number_left;
...
//    public void setGlVideoViewOnClickListener(GLVideoView.GlVideoViewOnClickListener glVideoViewOnClickListener) {
//        this.glVideoViewOnClickListener = glVideoViewOnClickListener;
//    }
//
//    private GLVideoView.GlVideoViewOnClickListener glVideoViewOnClickListener;
    ...
    public void setOnScrollClickListener(OnScrollClickListener l) {
        this.onScrollClickListener = l;
    }
    ...
    public void setLeftView(View leftView) {
        ...
        tv_user_number_left = (TextView) leftView.findViewById(R.id.tv_user_number_left);
        resetLeftView();
    }
    ...
    private void init() {
        ...
        gestureHandler.setGestureListener(new SDGestureHandler.SDGestureListener() {
            ...
            public boolean onVerticalScroll(MotionEvent e1, MotionEvent e2, float distanceX, float distanceY) {
                // 方便调试,禁止上下滑动切换房间,后端也没提供上下切换房间的逻辑暂时屏蔽
...
//                }
            ...
            public boolean onSingleTapUp(MotionEvent e) {
                // 判断是否点击了连麦窗
                int index = lickGlVideoViewcIndex(e);
                if (index > 0) {
                    LogUtil.e("clickevent: index = "+index);
//                    if(null!=glVideoViewOnClickListener){
//                       glVideoViewOnClickListener.onClick(index,QavsdkControl.getInstance().getAVUIControl().getGlVideoViewByIndex(index));
//                    }
                ...
                if (onScrollClickListener != null) {
                    onScrollClickListener.onClick(e);
                }
            ...
            public void onActionUp(MotionEvent event, float velocityX, float velocityY) {
                upX = event.getRawX();
                upY = event.getRawY();
                if (gestureHandler.isVerticalScroll()) {
                ...
                } else if (gestureHandler.isHorizontalScroll()) {
    ...
    // 判断点击区域是否是连麦区
    private int lickGlVideoViewcIndex(MotionEvent e) {
        float x = e.getX();
        float y = e.getY();

        //        if(QavsdkControl.getInstance().getAVUIControl() == null || QavsdkControl.getInstance().getAVUIControl().getGlVideoViewLayoutInfos() == null){
//            return 0;
//        }

//        AVUIControl.GLVideoViewLayoutInfo[] glVideoViewLayoutInfos = QavsdkControl.getInstance().getAVUIControl().getGlVideoViewLayoutInfos();

//        int videoCount = QavsdkControl.getInstance().getAVUIControl().getViewCount() - 1; // 要扣除自身
//
//        if (videoCount >= 1) { // 连麦窗口显示个数
//
//            if (x >= glVideoViewLayoutInfos[0].left && x <= glVideoViewLayoutInfos[0].right && y >= glVideoViewLayoutInfos[0].top && y <= glVideoViewLayoutInfos[0].bottom) {
//                return 1;
//            }
//        }
//        if (videoCount >= 2) {
//            if (x >= glVideoViewLayoutInfos[1].left && x <= glVideoViewLayoutInfos[1].right && y >= glVideoViewLayoutInfos[1].top && y <= glVideoViewLayoutInfos[1].bottom) {
//                return 2;
//            }
//        }
//        if (videoCount == 3) {
//            if (x >= glVideoViewLayoutInfos[2].left && x <= glVideoViewLayoutInfos[2].right && y >= glVideoViewLayoutInfos[2].top && y <= glVideoViewLayoutInfos[2].bottom) {
//                return 3;
//            }
//        }
        return 0;
            ...
            if (gestureHandler.getScroller().isFinished()) {
                switch (finishState) {
                    ...
                    case left:
                        horizontalIndex = 0;
                        if (listenerScroll != null) {
                            listenerScroll.onFinishLeft();
                        }
                        break;
                    case centerHorizontal:
                        horizontalIndex = 1;
                        if (listenerScroll != null) {
                            listenerScroll.onFinishHCenter();
                        }
                        break;
                    case right:
                        horizontalIndex = 2;
                        if (listenerScroll != null) {
                            listenerScroll.onFinishRight();
                        }
                        break;
