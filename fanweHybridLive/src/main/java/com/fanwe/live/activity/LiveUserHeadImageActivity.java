...
public class LiveUserHeadImageActivity extends BaseActivity {
	...
	private void getIntentData() {
		user_id = getIntent().getExtras().getString(LiveUserHomeActivity.EXTRA_USER_ID);
		url = getIntent().getExtras().getString(LiveUserHomeActivity.EXTRA_USER_IMG_URL);
	}
	
	/** 显示头像大图*/
	private void displayImage() {
		SDViewBinder.setImageView(this,url , iv_head_img,R.drawable.ic_default_head);
	}
	...
	private void savePicture() {
		showProgressDialog(getString(R.string.is_saving_pic));
		File dir = null;
		...
		x.http().get(params, new Callback.ProgressCallback<File>() {
			...
			public void onSuccess(File result) {
				...
				SDToast.showToast(getString(R.string.save_success));
				folderScan(dirPath);
			}
	...
	protected void onResume() {
		super.onResume();
		MobclickAgent.onPageStart("用户头像界面");
	...
	protected void onPause() {
		super.onPause();
		MobclickAgent.onPageEnd("用户头像界面");
