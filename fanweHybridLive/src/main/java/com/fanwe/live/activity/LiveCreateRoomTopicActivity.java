...
public class LiveCreateRoomTopicActivity extends BaseActivity implements OnEditorActionListener ,TextWatcher {
    ...
    private void initView() {
        ...
        view_topic.setOnTopicClickListener(new TopicClickListener() {
            ...
            public void onTopicClick(LiveTopicModel model) {
                //获取话题返回上一页面
                returnWithData(model.getTitle(),model.getCate_id(),model.getDesc());
            }
        });
    }
     ...
     */
    private void returnWithData(String topic,int cateId,String desc) {
...
data.putExtra(LiveCreateRoomActivity.EXTRA_DESC,desc);        startActivity(data);
    }
    ...
    public void onClick(View v) {
        ...
        switch (v.getId()) {
            case R.id.tv_ok:
                ...
                if(topic.equals("")) {
                    SDToast.showToast(getString(R.string.please_input_topic));
                    return;
                }
                returnWithData("#" + topic + "#",0,null);
                break;
            case R.id.iv_return :
                finish();
                break;
    ...
    protected void onResume() {
        super.onResume();
        MobclickAgent.onPageStart("创建房间-选择话题界面");
    ...
    protected void onPause() {
        super.onPause();
        MobclickAgent.onPageEnd("创建房间-选择话题界面");
