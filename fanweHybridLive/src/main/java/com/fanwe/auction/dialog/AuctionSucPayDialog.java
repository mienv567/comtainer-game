...
public class AuctionSucPayDialog extends SDDialogBase
{
    ...
    public AuctionSucPayDialog(Activity activity, PaiBuyerModel buyerModel)
    {
        super(activity);
    ...
    {
        AuctionSucPaySingleton.getInstance().addAuctionSucPaySingleton(new AuctionSucPaySingleton.OnTimerResultCallBack()
        {
            @Override
            public void timerResultCallBac(long day, long hour, long min, long sec)
            {
                if (min <= 0 && sec <= 0)
                {
    ...
    private void bindData()
    {
        ...
        if (buyerModel != null && user != null)
        {
            ...
            SDViewBinder.setImageView(getContext(),buyerModel.getGoods_icon(), iv_goods_pic);
    ...
    {
        Intent intent = new Intent(getOwnerActivity(), LiveRechargeActivity.class);
